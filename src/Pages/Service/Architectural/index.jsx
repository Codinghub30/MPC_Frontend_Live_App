...
const Architectural = () => {
  ...
  useEffect(() => {
    ...
    if (saved) {
      setServiceDetails((prev) => ({
        ...prev,
        housePlan: saved === "House Plan",
        interiorSpacePlan: saved === "Interior Space Plan",
      }));
    }
  }, []);
  ...
  const handleCheckboxChange = (event) => {
    ...
    setServiceDetails((prev) => {
      const updatedSelection = {
        housePlan: name === "housePlan" && checked,
        interiorSpacePlan: name === "interiorSpacePlan" && checked,
      };
      if (checked) {
        sessionStorage.setItem(
          "Service",
          name === "housePlan" ? "House Plan" : "Interior Space Plan"
        );
      } else {
  ...
  return (
    ...
    >
      ...
      >
        <div style={{ flex: "1 1 400px", minWidth: 300 }}>
          <h1 style={{ fontSize: 28, fontWeight: "bold", marginBottom: 15 }}>
            {selectedService || "Please select a Architectural Service Type"}
          </h1>
        ...
        >
          ...
          >
            <input
              type="checkbox"
              name="housePlan"
              checked={serviceDetail.housePlan}
          ...
          >
            <input
              type="checkbox"
              name="interiorSpacePlan"
              checked={serviceDetail.interiorSpacePlan}
              onChange={handleCheckboxChange}
              style={{ marginRight: 10, width: 24, height: 24 }}
            />
            <span
      ...
      {serviceDetail.interiorSpacePlan ||
        (serviceDetail.housePlan && (
          <>
            <div
              style={{
                maxWidth: 800,

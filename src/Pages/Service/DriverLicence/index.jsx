...
const DriverLicense = () => {
  ...
  useEffect(() => {
    ...
    setDriverLicense(saved);
  }, []);

  const handleCheckboxChange = (event) => {
    const { name, checked } = event.target;
    if (name === "driverLicense") {
      setDriverLicense(checked);
      if (checked) {
        sessionStorage.setItem("Service", "New Driver License");
      } else {
        sessionStorage.removeItem("driverLicense");
      }
    } else if (name === "driverLicenseCorrection") {
      setDriverLicenseCorrect(checked);
      if (checked) {
        sessionStorage.setItem("Service", "Driving License Correction");
      } else {
      ...
      <div style={{ padding: "30px 80px", maxWidth: 600, margin: "40px auto" }}>
        ...
        >
          ...
          >
            <input
              type="checkbox"
              name="driverLicenseCorrection"
              checked={driverLicenseCorrect}
              onChange={handleCheckboxChange}
              style={{ marginRight: 10, width: 24, height: 24 }}
            />
            <span

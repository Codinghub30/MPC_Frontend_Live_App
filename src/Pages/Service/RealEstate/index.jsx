...
const RealEstate = () => {
  ...
  const [propertySale, setPropertySale] = useState(false);
  const [propertyBuy, setPropertyBuy] = useState(false);

  // Check if sessionStorage has saved states for checkboxes
  useEffect(() => {
    const savedPropertySale = sessionStorage.getItem("propertySale") === "true";
    const savedPropertyBuy = sessionStorage.getItem("propertyBuy") === "true";

    setPropertySale(savedPropertySale);
    setPropertyBuy(savedPropertyBuy);
  }, []);

  const handleCheckboxChange = (event) => {
    const { name, checked } = event.target;
    if (name === "propertySale") {
      setPropertySale(checked);
      sessionStorage.setItem("Service", checked);
    } else if (name === "propertyBuy") {
      setPropertyBuy(checked);
      sessionStorage.setItem("Service", checked);
    }
  };
      ...
      >
        <div style={{ flex: "1 1 400px", minWidth: 300 }}>
          <h1 style={{ fontSize: 28, fontWeight: "bold", marginBottom: 15 }}>
            {selectedService || "Please select a Real Estate Service Type"}
          </h1>
